...
This service calls compute_total(price, quantity, discount, tax)
to calculate line items for invoices.
"""


def calculate_invoice_line(price, quantity, discount=0, tax=0):
    """Calculate a single invoice line item.

    Uses the same compute_total(price, quantity, discount, tax)
...
def generate_bill(items, tax=0.08):
    """Generate a bill from a list of items."""
    total = 0
    lines = []
    for item in items:
        amount = calculate_invoice_line(
            item["price"], item["quantity"], item.get("discount", 0), tax
        )
